...
class Daemon:
    ...
    def daemonize(self):
        ...
        See Stevens' "Advanced Programming in the UNIX Environment" for details (ISBN 0-201-56314-X)
    ...
    def run(self):
        ...
        """
        while True:
            time.sleep(1) # example, replace with your daemon's code.
...
if __name__ == "__main__":
    class MyDaemon(Daemon):
        def run(self):
            while True:
                time.sleep(1)
                with open("/tmp/testdaemon.log", "a+") as f:
                  f.write(f"{time.ctime()}\n")

    daemon = MyDaemon('/tmp/mydaemon.pid', stdout="/tmp/mydaemon.log")
    if len(sys.argv) == 2:
        if 'start' == sys.argv[1]:
            daemon.start()
